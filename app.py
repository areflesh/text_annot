...
if st.session_state.file_uploaded and st.session_state.captions:
    ...
    # If this caption is already annotated, pre-fill the form
    if str(current_idx) in st.session_state.annotations:
        existing_anno = st.session_state.annotations[str(current_idx)]
        if 'subject' not in st.session_state or st.session_state.subject == "":
            st.session_state.subject = existing_anno['subject']
        if 'predicate' not in st.session_state or st.session_state.predicate == "":
            st.session_state.predicate = existing_anno['predicate']
        if 'object' not in st.session_state or st.session_state.object == "":
    ...
    with col2:
        predicate = st.text_input("Predicate", key="predicate")
    with col3:
        object = st.text_input("Object", key="object")
